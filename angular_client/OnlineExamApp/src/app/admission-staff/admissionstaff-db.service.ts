...
//cast observable result into an array of students by using the Istudent interface
  stu: Istudent[] = []; 
  ...
  getStudents() {
    ...
    this.http.get<any>(url) // where we can make api call to node js
      .subscribe((res) => { //since return type is observable we are listening to it by subscribing 
        res.forEach(element => {
